...
public List<Account> getAccountsBySignerId(int signerId) {
    ...
    public User createUser(User newUser) {
        String sql = "INSERT INTO users (username, password_hash, role, signer_id) VALUES (?, ?, ?, ?) RETURNING user_id";

        try {
            String hashedPassword = passwordEncoder.encode(newUser.getHashedPassword());
    ...
    public Signer getSignerBySignerSsn(String ssn) {
        ...
        if (rs.next()) {
            return new Signer(rs.getInt("customer_id"), rs.getString("name"), rs.getString("ssn"));
        }
        return null;
    }
    ...
    public List<Account> getAccountsBySignerSsn(String ssn) {
        ...
        String sql = "SELECT * FROM accounts WHERE primary_signer = ? OR secondary_signer = ?";
    ...
    public void transferBalance(String fromAccount, String toAccount, double amount) {
        ...
        jdbcTemplate.update(sql1, amount, toAccount);
        jdbcTemplate.update(sql2, amount, fromAccount);
    }
